...
export function RegistryExplorerView({
  ...
  const handleToggleVoiceListening = () => {
    ...
    if (!SpeechRecognition) {
      simulateVoiceQuery('ASTM A106 Seamless');
      return;
    }
    ...
    try {
      ...
      recognition.onstart = () => {
        setIsListening(true);
        setVoiceTranscript('Listening... Speak material specification...');
      };
    ...
    <div className="space-y-4 max-w-[1750px] mx-auto font-sans">
      ...
      <div className="bg-white border border-slate-200/90 rounded-xl p-4 shadow-2xs space-y-3">
        <div className="relative w-full">
          ...
          <input
            type="text"
            placeholder={isListening ? voiceTranscript : "Search by National Code, Description, Standard, CPSE Code, Local Name, Plant, Size (e.g. NMM-0001842, ASTM A106)..."}
            value={searchQuery}
            onChange={(e) => {
              setSearchQuery(e.target.value);
              setCurrentPage(1);
            }}
            className={`w-full bg-slate-50 border ${isListening ? 'border-amber-400 ring-2 ring-amber-100' : 'border-slate-200'} rounded-lg pl-10 pr-12 py-2 text-xs font-normal text-slate-900 placeholder:text-slate-400 focus:outline-blue-500 transition-colors`}
          />
          <button
            onClick={handleToggleVoiceListening}
            className={`absolute right-2 top-1/2 -translate-y-1/2 p-1.5 rounded-md transition-colors cursor-pointer ${
              isListening ? 'bg-amber-100 text-amber-600 animate-pulse' : 'text-slate-400 hover:text-slate-700 hover:bg-slate-100'
            }`}
